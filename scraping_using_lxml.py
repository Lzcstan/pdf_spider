...
def get_link_xpath(html):
    try:
        ...
        #     # a = tree.xpath('//div[@id="buttons"]/ul/li/li/a') # tid: FIXME
        #     a = tree.xpath('//div[@id="buttons"]')
        #     # print(">>>>>>", a.)
        ...
        # for a_unit in a:  # 寻找带有onclick属性的<a>标签 # Tid: FIXME
        #     onclick = a_unit.get('onclick')
        #     if onclick:
        #         break
        # # print(onclick)
        # onclick = re.findall(r"location.href\s*=\s*'(.*?)'", onclick)[0]
        onclick = re.findall(r"location.href\s*=\s*'(.*?)'", html)[0]
        flag = True  # 执行位置标记
