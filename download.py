...
def download(url, headers, proxies=None, num_retries=2, timeout=30):
        ...
        try:
            ...
            if resp.status_code >= 400:
                ...
                if num_retries and 500 <= resp.status_code < 600:
                    return download(url, headers, proxies, num_retries-1, timeout)
            break
        except (requests.exceptions.RequestException, requests.exceptions.Timeout) as e:
            elapsed_time = time.time() - st_time
            if elapsed_time > timeout:
                print(f'Download error, download took too long: {elapsed_time}s, trying again.')
                continue
            else:
...
def download_pdf(start_url, result, headers, dir, proxies=None, num_retries=2, doi=None, timeout=60):
    """
    ...
    while True:
        try:
            ...
            if resp.status_code >= 400:
                ...
                if num_retries and 500 <= resp.status_code < 600:
                    return download_pdf(start_url, result, headers, dir, proxies, num_retries-1, doi, timeout)
                else:
                    return False
            break
        except (requests.exceptions.RequestException, requests.exceptions.Timeout) as e:
            ...
            if elapsed_time > timeout:
                print(f'File download error, download took too long: {elapsed_time}s, trying again.')
                continue
            else:
...
def sci_hub_crawler(id2doi, dir, robot_url=None, user_agent='sheng', proxies=None,num_retries=2,
                delay=1.5, start_url='sci-hub.do', useSSL=True, get_link=None, nolimit=False, cache=None):
    """
    ...
    for eid, doi in id2doi.items():
        ...
        if rp and rp.can_fetch(user_agent, url) or nolimit:
            cur_delay = random.uniform(0.5, delay)
            wait(url, cur_delay, domains)

            html = download(url, headers, proxies, num_retries)
            result = get_link(html)

            if result:
                result['title'] = doi.replace('/', '_')
                if download_pdf(start_url, result, headers, dir, proxies, num_retries, doi):
                    if cache:
                        cache[url] = 'https:{}'.format(result['onclick'])  # cache
                    download_succ_cnt += 1
            else:
                print('Not in Sci-hub for URL: ', url)
            print(f'{eid}: delay {cur_delay}s')
        else:
            print('Blocked by robots.txt: ', url)
    print('%d of total %d pdf success' % (download_succ_cnt, len(id2doi)))
